...
State the goal of the system, the main user or business value, and the scope supported by the repository evidence.

The codebase is scoped to `{architecture_scope_statement}`.

If repository evidence is thin, say so and keep the description bounded.
...
Describe the major runtime components, their boundaries, and how they interact.
