...
This agent performs structured ADA and WCAG analysis for React code, pages, flows, domains, and full codebases. It produces a structured JSON report that separates automated findings, manual findings, and user-flow findings so stakeholders can act on the results without confusing static evidence with runtime validation needs.
...
- Use it when you need an executive-ready report with prioritization and remediation guidance.
